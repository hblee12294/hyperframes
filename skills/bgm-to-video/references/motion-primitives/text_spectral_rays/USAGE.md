# Using `text_spectral_rays` — it OWNS its wordmark

`text_spectral_rays` is a **self-contained WebGL hero-text renderer**. From ONE rasterized
...
A background bed (`bg_flow_field`) or **separate** supporting elements (tagline, CTA, rule)
